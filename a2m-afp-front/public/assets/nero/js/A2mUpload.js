...
function getLang() {
    ...
    if (userLang != 'ko' && userLang != 'en' && userLang != 'jp' && userLang != 'de') {
        userLang = 'ko'
    }
    return userLang;
}
    ...
    $.fn.A2mUpload = function (opts) {
        ...
        var A2mDms = function (opts) {
            ...
            $context.data = [],
                ...
                $context.upload = function (callback) {
                    if (callback) {
                        callback();
                    }
                    try {
...
//     			          $('.largefileupload-drop').show(); - JM
            ...
            var fileForm = function (file, obj) {
                ...
                if ($context.getOpt('dropAreaView') === true) {
                    ...
                    // var fileAction = '<a href="javascript:void(0);" onclick="a2mDmsUI.removeFileByUIdentifier(\'' + file.uniqueIdentifier + '\')" class="progress-remove-link-' + file.uniqueIdentifier + '"><img src="' + $context.getOpt('contextPath') + $context.getOpt('closeImageUrl') + '" title="Delete upload" height="10px" width="10px"/></a>';

//				fileAction = fileAction + '<a href="javascript:void(0);" onclick="a2mDmsUI.downloadFileByUIdentifier(\''+file.uniqueIdentifier+'\')" class="btn btn-info btn-xs progress-download-link-'+file.uniqueIdentifier+'" style="display:none;">Download</a><br/>';
//				fileAction = fileAction + '<a href="javascript:void(0);" onclick="a2mDmsUI.deleteFileByUIdentifier(\''+file.uniqueIdentifier+'\')" class="btn btn-warning btn-xs progress-delete-link-'+file.uniqueIdentifier+'" style="display:none;">Delete</a>';
                    ...
                    $(obj).find('.largefileupload-delete').on('click', function () {
                        swal(comMessage.DeleteConfirm, function () {
                            if($context.getData()[0].isUploaded) {
                                $context.removeDataByUIdentifier($context.getData()[0].obj['uniqueIdentifier']);

                                let onDeleteComplete = $context.opts['onDeleteComplete'];
                                if (onDeleteComplete) {
                                    onDeleteComplete($context, deleteData);
                                }

                                // do not need delete on disk now
            ...
            $context.deleteFileByUIdentifier = function (uid, events) {
                ...
                try {
                    ...
                    swal(comMessage.DeleteConfirm,
                        function () {
                            $context.removeDataByUIdentifier(uid);

                            let onDeleteComplete = $context.opts['onDeleteComplete'];
                            if (onDeleteComplete) {
                                onDeleteComplete($context, [file]);
                            }
                            // now do not need delete on disk
...
//	              this.a2mDmsMotion.setProgress(pLfu, file, this.mA2mDms);
    ...
    this.fileForm = function (pLfu, file, obj) {
        ...
        var fileAction = '<a href="javascript:void(0);" onclick="var a2mDmsUI = $(\'.a2m-dms-dialog\').data(\'dmsUI\');a2mDmsUI.pause();swal(comMessage.DeleteConfirm, function(isConfirm){if(isConfirm){a2mDmsUI.removeFileByUIdentifier(\'' + file.uniqueIdentifier + '\',' + pLfu.largeFileUploadId + '); swal.close();}else{a2mDmsUI.upload(); $(\'.progress-pause\').show(); $(\'.progress-resume\').hide();}});" class="progress-remove-link-' + file.uniqueIdentifier + '"><img src="' /*+ $context.getOpt('contextPath')*/ + $context.getOpt('design').uploadPopup.deleteImg + '" title="Delete upload" height="20px" style="margin: auto; display: block;" alt="업로드 취소"/></a>';
//				fileAction = fileAction + '<a href="javascript:void(0);" onclick="a2mDmsUI.downloadFileByUIdentifier(\''+file.uniqueIdentifier+'\','+pLfu.largeFileUploadId+')" class="btn btn-info btn-xs progress-download-link-'+file.uniqueIdentifier+'" style="display:none;">Download</a><br/>';
//				fileAction = fileAction + '<a href="javascript:void(0);" onclick="a2mDmsUI.deleteFileByUIdentifier(\''+file.uniqueIdentifier+'\','+pLfu.largeFileUploadId+')" class="btn btn-warning btn-xs progress-delete-link-'+file.uniqueIdentifier+'" style="display:none;">Delete</a>';
        $(obj).find('.largefileupload-action-' + file.uniqueIdentifier).html(fileAction);
        $(obj).find('.largefileupload-file-' + file.uniqueIdentifier + ' .largefileupload-file-name').append(file.fileName);
    ...
    this.buildUploadBox = function () {
        ...
        if ($('#a2m-dms-ui').length == 0) {
            ...
            res = res + '<div id="a2m-dms-uploadbox" > <div id="a2m-dms-frame"> '
                ...
                + '<div>'
                + '<div class="progress-resume" style="display: none"><a href="javascript:void(0);" ngocdb   onclick="var a2mDmsUI = $(\'.a2m-dms-dialog\').data(\'dmsUI\');a2mDmsUI.upload(); $(\'.progress-pause\').show(); $(\'.progress-resume\').hide(); return(false);" class="progress-resume-link">전송시작</a></div>'
                + '<div class="progress-pause"><a href="javascript:void(0);" onclick="var a2mDmsUI = $(\'.a2m-dms-dialog\').data(\'dmsUI\');a2mDmsUI.pause(); return false;" class="progress-pause-link">전송 중단</a></div> '
                + '</div>'
