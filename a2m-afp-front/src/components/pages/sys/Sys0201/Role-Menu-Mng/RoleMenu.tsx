...
export default function RoleUser() {
  ...
  const [listRoleMenu, setListRoleMenu] = useState<any>([]);
  ...
  const changeSelectItem = (itemId: any) => {
    ...
    Sys0201Service.getInstance()
      .search_menu_role({ roleId: itemId })
      .then((response) => {
        setListRoleMenu(response.data.responseData);
  ...
  const setChecked = (checked: any, element: any, type: number) => {
    let _listRoleMenu = [...listRoleMenu];
    ...
    if (checked) {
      ...
      var p = _listRoleMenu.find((ele) => ele.menuId === element.menuId);
      switch (type) {
        case 1: {
          element.readYn = "N";
          if (p !== undefined) {
      ...
      _listRoleMenu = _listRoleMenu.filter(
        (ele) => ele.menuId !== element.menuId
      );
    }
    const item = {
      roleId: selectedItem,
    ...
    _listRoleMenu.unshift(item);
    setListRoleMenu(_listRoleMenu);
  };

  const save = () => {
    let _listRoleMenu = listRoleMenu.reverse();
    originLstMenuRole.forEach((item: any) => {
      if (
        !_listRoleMenu.some((element: any) => element.roleId === item.roleId)
      ) {
        item["checked"] = false;
        _listRoleMenu.push(item);
      } else {
      }
    });
    Swal.fire({
        ...
        <div className="card-body">
          <div className="d-flex justify-content-between mb-3">
            <div className='title-head-body'>
              <h2 className="mb-sm-0 card-title mb-0 flex-grow-1">
                {t("sys0201.title.table.roleMenuMng")}
              </h2>
            </div>
                ...
                <table className="table table-hover mb-3">
                  <thead className="table-light">
                    <tr>
                      ...
                      <th scope="col" rowSpan={2}>
                        {t("sys0101.table.header.menuId")}
                      </th>
                      <th scope="col" rowSpan={2}>
                        {t("sys0101.table.header.menuNm")}
                      </th>
                  ...
                  <tbody>
                    {listMenu.map((element: any, index: number) => (
                      <tr key={element.menuId} className="cus-cursor">
